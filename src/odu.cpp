...
vec rk6_function_solve ( float a, float b, float h, int n, float ( *f ) ( float, float ), float y0 )
    {
    float k1, k2, k3, k4, k5, k6;
    vec y ( n+1 );
...
matr rk6_system_function_solve_vector ( float a, float b, float h, int n, vec ( *vf ) ( float, vec ), vec vs )
    {
...
float new_point_fahlberg ( float ( *f ) ( float, float ), float t, float y, float h, float &y_new )
    {
...
float new_point_DoPr ( float ( *f ) ( float, float ), float t, float y, float h, float &y_new )
    {
    float k1, k2, k3, k4, k5, k6, k7, high, low;
...
float new_hop ( float h, float loc_err, float tol, float min_h, float max_h )
    ...
    float new_h = std::max ( (float) (fac * h * pow ( ( tol/loc_err ), 1./ ( p+1 ) )), min_h );
...
matr rk54 ( float a, float b, float ( *f ) ( float, float ), float y0, float tol, float min_h, float max_h )
    {
    float loc_t = a, dt = tol, new_dt, err, y_new;
